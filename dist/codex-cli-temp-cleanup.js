...
export async function cleanupCodexRuntimeTempRoot(input) {
    const secretsScrubbed = await scrubSensitiveCodexHomePaths(input.tempCodexHome);
    try {
...
export function isTransientCodexTempCleanupError(error) {
    ...
    return (message.includes("directory not empty") ||
